...
public:
  ...
  void import_bowtie_alignment(string& alignment_string, ofstream& error_str, bool using_error_stream){
	...
	//cout<<"fields: "<<alignment_string_fields.size()<<endl;
	//cout<<"chrom: "<<chrom<<" coord: "<<coord<<endl;
...
int main(int argc, char* argv[]){
  ...
  pars.require("align_type", "eland/eland_extended/QuEST/solexa_align/bowtie", STRING_TYPE);
  ...
  if(align_type != "eland" && align_type != "QuEST" && align_type != "solexa_align" && align_type != "eland_extended" && align_type != "bowtie" && align_type != "maq"){
    cerr<<"Bad align type. Expected eland/eland_extended/QuEST/solexa_align/bowtie/maq but found "<<align_type<<". Aborting."<<endl;
    exit(4);
  }
